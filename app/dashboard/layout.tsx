...
import { LayoutDashboard, ShoppingBag, UtensilsCrossed, Users, LogOut, Lock, BarChart3, Eye, EyeOff, Monitor, CreditCard, Tag, Star } from 'lucide-react'
...
const NAV = [
  ...
  { href: '/dashboard/puntos',      label: 'Puntos',       icon: Star },
]
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-20 bg-gray-900 border-t border-gray-800 flex overflow-x-auto no-scrollbar">
        {NAV.map(({ href, label, icon: Icon, exact }) => {
          const active = exact ? pathname === href : pathname.startsWith(href)
          return (
            <Link key={href} href={href}
              className={`shrink-0 flex flex-col items-center gap-0.5 py-2 text-[10px] font-bold transition ${active ? 'text-[#006B42]' : 'text-gray-500'}`}
              style={{ minWidth: 64 }}>
